...
每个 `books/<slug>/` 是独立本地 Git 工作区。书内 `.git` 是 gitdir 指针，真实
元数据位于主仓库忽略的 `.local-book-git/<slug>.git`；不得配置 remote、push 或
上传小说历史。`book-git-status` 只返回 head、dirty、remote_count 等元数据。

generation 绑定后自动创建 `chapter: chNN draft`，章节进入 ready 后自动创建
`chapter: chNN ready`。第 5/10/15... 章 ready 后创建本地 checkpoint tag。
Git 只负责 diff、恢复和实验回放，不替代 evidence、ready、作者批准或发布许可。
实验项目要求彻底清除原文时，必须同时清除 `books/<slug>` 与对应外置 gitdir；
只删工作区会保留完整历史。
...
- 每份 generation 的运行指标只填本次增量，不把会话累计总数重复复制到多份记录。
  ...
  一章 runtime audit 只能绑定当前 generation；同一 writer `run_id` 不得跨章复用，
  draft mutation 与 review call 计数缺失或超限都会阻断 ready。
...
`sync-tools` 会受控迁移带 v3.7/v3.8/v3.9/v4.0/v4.1/v4.2 生成标记的
`CLAUDE.md`/`README.md`，补齐 v4.3 读者追读、运行真相、每书本地 Git 与章节序列，
