...
1. `project-status` + `evidence-status`，确认上一章无 blocking；再用
   `set-draft-mode` 固定 `formal | exploration | degraded_exploration`。写作 Harness
   必须按通用契约报告真实 session id；不能报告时不得把自报指标当作正式审计。
...
blocking；句长塌缩和低量精确复读仍是 advisory。机器不认证文学价值。
...
常用只读 op：`harness-contract`、`project-status`、`run-gates`、`evidence-status`、
`memory-status`、`session-audit`。
常用变更 op：`set-draft-mode`、`record-evidence`、`record-review`、
`record-session-audit`、`advance-state`、`sync-tools`、memory candidate/promotion。

`sync-tools` 会受控迁移带 v3.7/v3.8 生成标记的 `CLAUDE.md`/`README.md`，并把旧状态
