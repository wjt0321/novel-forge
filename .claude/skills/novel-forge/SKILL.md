...
3. 只交付动作内上下文并等待宿主官方终态。
4. 把终态、typed operation handle、真实 Session/模型和 `role_result` 写入仓库外
   JSON，运行 `complete-role <slug> --from-file <临时JSON>`。
...
Python 读取 `harness-contract` 与 `guardian-contract`，用 `session-audit` 校验
`novel-forge-runtime/v1`；`continue_allowed=false` 即停机。正式结果统一为
含 `role` 与 `payload` 的 `novel-forge-role-result/v1`。

### 隔离 Writer Capsule

Formal writer 不进入 `books/<slug>/`。默认 `formal_native` 使用仓库外 Capsule、
角色零项目写入和执行前后全仓快照；宿主具备真实文件系统沙箱时可透明升级为
`formal_sandboxed`。这是控制面保证级别，不是用户 A/B 选择。writer
只见合同、`handoff.md`、`formal-writer/v1` 的 `instructions.md` 和
...
`idle/available` 或文件稳定无效。超时或结果无效保留失败回执并换新
...
7. 默认只运行两角色：blind-reader 与 chapter-editor。blind-reader 必须在不同于
   ...
   代填。三角色须有不同 `session_instance_id` 和外置完成凭证。不得暂停询问审核。
    审稿 completed 但没有正式结果通道、`role_result` 或角色绑定时，废弃该审稿
    session 并用新 session 自动重试，最多两次；Blind 已成功时 Editor 故障只重试
    Editor。耗尽后才显示 A/B/C。无法创建独立审稿会话时返回
...
`sync-tools` 只迁移带版本标记的生成文件到 v5.1；手写项目宪法不动，旧专业 Agent
仅作兼容资产。
