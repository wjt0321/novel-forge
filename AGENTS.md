...
| `native_relay.py` | v5.3 原生会话拉取/回传协议：默认 `lean_native`，角色只写动作指定的正文或 `result_file`，Lead 只回传真实 session ID；Python 自动生成哈希、Generation、Runtime、Guardian、状态与 Git。`--strict-audit` 保留 v5.2 完成信封与全仓快照。 |
...
- v5.3 正文优先完成门：Lean 动作不携带 `completion_template`。规划和审稿角色只写外置 `result_file`，Writer 只写 Capsule 正文，Lead 只回传真实 session ID。Python 自动计算哈希、Generation、stale、Review 绑定、Runtime、Guardian、状态与 Git。技术字段缺失不得导致有效正文重写；有效 Generation 后的审稿运输失败只从当前审稿角色恢复。严格审计模式继续接受旧完整完成信封。
