...
| `workflow.py` | 厂商无关的自动三角色轻量编排器：宿主通过 `SessionBackend` 提供真实独立会话；Writer 自己产出允许列表内的规划并写正文，两个审稿角色返回各自实质判断；编排器等待异步 Writer 正文/runtime 稳定，注入用户硬锚合同，并完成 Story Brief 过滤、Guardian、generation、runtime、双审、集中 Patch、人工授权回炉、状态与每书 Git 闭环，不代写、代审或选择模型 |
...
- `docs/`：33 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相、隔离 Writer Capsule、编译 Writer Prompt、自动三角色工作流、文学生产闭环、异步会话完成协议与文学微规则等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
