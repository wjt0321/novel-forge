...
| `session_audit.py` | v4.3 厂商无关 Harness Contract：定义 `novel-forge-runtime/v1` 累计快照、分层推理策略、硬停预算、自动审稿编排与每书本地 Git 策略；具体产品日志解析仅为兼容导入 |
...
- `books/<slug>/`：**新前台工作流**（`init-novel-project` 生成）。**整个 `books/` 目录已被 Harness 主仓库 gitignore，仅存本地、不随主仓库上传**。每本书同时拥有独立本地 Git；书内 `.git` 是指向 `.local-book-git/<slug>.git` 的 gitdir 指针，且不允许配置 remote。正文唯一入口是 `chapters/eXX/ch-XX/正文.md`；`memory/canon/**/*.md` 是本书连续性权威源，`memory/candidates/` 是待审增量，`.novel-forge/index.sqlite3` 是按书生成且可删除重建的检索缓存；`planning/chapter-sequences/*.json` 是可更新的章节编排状态；`memory/context-cache/chXX-handoff.md` 是可删除的有界会话交接包；`evaluation/` 保存评测宪法与实验模板，`evidence/` 保存不可变创作证据及 `runtime-audits/` 脱敏运行审计。各书自包含，资产不得跨项目共享。
- `.local-book-git/<slug>.git`：每书外置本地历史，主仓库同样 gitignore。删除书工作区后可据此恢复；实验样本需要彻底清除时必须与 `books/<slug>/` 一并删除。
...
books/ v4.3 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- v4.3 读者追读与运行真相门：blind-reader pass 必须证明真人愿意继续读；同一 writer `run_id` 不得跨章复用；runtime audit 只能绑定当前 generation 且 `scope_chapter_count=1`；draft mutation/review call 计数缺失或超限都阻断 ready。formal surface gate 通过后 Harness 必须自动创建独立审稿会话，不得暂停询问是否审核；无法创建时返回 `review_session_required`。
...
- 若用户要求彻底删除某个实验书，先保存允许保留的聚合证据，再验证并删除 `books/<slug>/` 与 `.local-book-git/<slug>.git` 两个绝对路径；只删除其中一个不算彻底清理。
- 正式短篇章节硬门槛：≥ 5000 个 CJK 汉字（用户硬性要求，不可下调）；正式小说不得使用 `--allow-below-minimum`。
- 不得在正文（`正文.md`）中写入提示词、工作流标记、研究笔记或 Agent 身份；未经用户明确批准不得宣称"已批准"。

## 其他目录速览

- `docs/`：28 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
