...
- v5.4 暂存正文优先完成门：Lean 动作不携带 `completion_template`。Python 在后台生成最小规划材料；Writer 不回传规划表，只在当前书 diff 区写初稿或集中修订。blocking 表面规则未清理时最多三次复用同一暂存正文，不得在第一次残留后直接锁死任务。Blind Reader 与 Chapter Editor 都读取当前暂存正文，只写各自 `result_file`。Lean Chapter Editor 只需 `verdict`、`must`、`summary`、`evidence_quote`，通用 `pass` 等价于内部 `ready_for_editor_decision`；`analysis` 与 `hard_anchor_coverage` 只属于 strict audit。双审通过前不得创建正式章节、Generation 或 Guardian Receipt；通过后由 Python 一次性 CAS 晋升到 `chapters/`，自动计算哈希、Generation、Review 绑定、Runtime、Guardian、状态与 Git。Lead 无需填写技术表单或拼装 session ID。审稿运输失败只重开当前审稿角色，不重写正文；严格审计模式继续接受完整完成信封。
