...
books/ v3.4 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- v3.4 人类叙事证据门：分支必须引用盲评并选择单一胜者；作者偏好必须引用同一分支/盲评且不得修改 Canon；同模型换角色名不算独立审稿；来源元数据不实的样本不得参与模型比较。
...
- `docs/`：19 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知与因果归属等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
