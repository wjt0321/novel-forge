...
| `workspace_integrity.py` | 创作角色零项目写入护栏：Lean 默认只检查当前书，strict audit 使用全仓库快照；清理确认的新建泄漏并阻断既有文件修改/删除 |
...
| `review_capsule.py` | 仓库外审稿输入封存：按角色最小文件集、正文与 manifest 哈希绑定、回传前完整性复核；Lead 只传路径，不搬运正文 |
...
| `native_relay.py` | v5.3 原生会话接力协议：默认 `lean_native`，首个 Writer 动作直接写正文，审稿角色只写动作指定的 `result_file`；Python 用内部 control run 记录哈希、Generation、Runtime、Guardian、状态与 Git，Lead 无需填写技术表单或拼装宿主 session ID。`--strict-audit` 保留 v5.2 完成信封与全仓快照。 |
...
- v4.4/v5.3 隔离 Writer Capsule 门：formal Agent writer 必须在仓库外一次性 capsule 中运行。默认 `lean_native` 由原生独立 Session、Capsule 输出清单、当前书写入检查和 Guardian 绑定构成；未知 runtime 遥测保持 null，由 Python 记录为 unassessed，不阻断正文或 ready。`--strict-audit` 保留 `formal_native`/`formal_sandboxed` 完整隔离证明、预算观测和全仓快照。writer 只能接收 `capsule.json`、`guardian-contract.json`、`instructions.md`、`handoff.md`，只能产出 `draft/正文.md`；不得自写 runtime、回执或控制面。额外脚本、软链接/路径逃逸、保护输入变化或 session 不一致仍标记为 compromised。新 capsule 会淘汰旧 prepared capsule；patch 使用正文哈希 CAS。第三个潜在正文版本仍需签名授权。
- v5.3 正文优先完成门：Lean 动作不携带 `completion_template`，首个 Writer 动作直接写 Capsule 正文；Python 在后台生成最小规划材料，Writer 不回传规划表。审稿角色只写外置 `result_file`，Lead 等官方终态后执行 `complete-role`，无需填写技术表单或拼装宿主 session ID。Python 自动计算哈希、Generation、stale、Review 绑定、Runtime、Guardian、状态与 Git。技术字段缺失不得导致有效正文重写；有效 Generation 后的审稿运输失败只从当前审稿角色恢复。严格审计模式继续接受旧完整完成信封。
...
- `planning/guardian-sessions/*.json` 只能经 Guardian adapter 操作更新；`evidence/guardian-receipts/*.json` 是不可变导入证据，不得覆盖。正式 capsule 必须位于仓库外；Lean 默认验证 Capsule 输出与当前书写入范围，strict audit 才要求完整遥测和全仓快照。无法创建独立 Session 或取得官方终态时必须停止，只有用户明确要求探索稿时才能进入 `degraded_exploration`。
...
- `docs/`：43 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相、隔离 Writer Capsule、编译 Writer Prompt、自动三角色工作流、文学生产闭环、异步会话完成协议、文学微规则、会话产物封存、Harness 信任、原生角色终态、带类型句柄与角色结果通道、Python 确定性控制、原生 Relay、完成补交、硬锚会话完整性与正文优先 Lean 工作流等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
