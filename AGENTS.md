...
# 自动写作请求的首个写操作只能是 start；Backend 不可用时本章未开始，禁止自行降级
...
| `workflow.py` | 厂商无关的自动三角色轻量编排器：宿主通过 `SessionBackend` 提供真实独立会话；命令桥只接受仓库外入口，固定入口哈希并在每次调用前后核对控制面；Writer 自己产出允许列表内的规划并写正文，两个审稿角色返回各自实质判断；编排器等待异步 Writer 正文/runtime 稳定，注入用户硬锚合同，并完成 Story Brief 过滤、Guardian、generation、runtime、双审、集中 Patch、人工授权回炉、状态与每书 Git 闭环，不代写、代审或选择模型 |
...
- `planning/guardian-sessions/*.json` 只能经 Guardian adapter 操作更新；`evidence/guardian-receipts/*.json` 是不可变导入证据，不得覆盖。正式 capsule 必须位于仓库外，且外部 Harness 必须实际限制 writer 文件系统；自动生产无法限制时必须停止，只有用户明确要求探索稿时才能进入 `degraded_exploration`。
...
- `docs/`：36 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相、隔离 Writer Capsule、编译 Writer Prompt、自动三角色工作流、文学生产闭环、异步会话完成协议、文学微规则、会话产物封存与 Harness 信任等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
