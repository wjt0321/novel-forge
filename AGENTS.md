...
- 原生角色完成只认宿主官方 wait/join 终态；created/accepted/progress 或文件稳定不算完成。创建后必须保存宿主返回的真实 task/agent ID，禁止把角色名当 TaskOutput ID、固定 sleep 或提前 stop；默认至少等待 30 分钟，working/progress 状态继续等待。Blind Reader 正式落盘后才创建 Chapter Editor；退役 session 的晚到产物无效。模型配置只是偏好，正式来源只认终态返回的实际 resolved model。
...
- `docs/`：37 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相、隔离 Writer Capsule、编译 Writer Prompt、自动三角色工作流、文学生产闭环、异步会话完成协议、文学微规则、会话产物封存、Harness 信任、原生角色终态与模型选择等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
