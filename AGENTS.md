...
| `session_audit.py` | v3.9 厂商无关 Harness Contract：定义 `novel-forge-runtime/v1` 累计快照，脱敏统计 token/请求/缓存/工具失败/上下文重置，校验 generation 来源并给出硬停决定；具体产品日志解析仅为兼容导入 |
...
- `books/<slug>/`：**新前台工作流**（`init-novel-project` 生成）。**整个 `books/` 目录已 gitignore，仅存本地不上传**。正文唯一入口是 `chapters/eXX/ch-XX/正文.md`；`memory/canon/**/*.md` 是本书连续性权威源，`memory/candidates/` 是待审增量，`.novel-forge/index.sqlite3` 是按书生成且可删除重建的检索缓存；`evaluation/` 保存评测宪法与实验模板，`evidence/` 保存不可变创作证据及 `runtime-audits/` 脱敏运行审计；另有 `voice-bible.md`、一页式场景包、`reviews/`、`patches/`、薄壳工具、四个默认 Agent 角色与本书宪法。旧专业审稿角色仍可按需使用，但不再默认生成。各书自包含，资产不得跨项目共享。
...
books/ v3.9 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- `docs/`：24 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环与外置 Harness 护栏等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
