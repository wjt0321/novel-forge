...
# v4.4 隔离 writer op：guardian-contract / authorize-regeneration / prepare-writer-capsule / record-capsule-runtime / ingest-writer-capsule / invalidate-chapter-session
...
books/ v4.4 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- v4.4 隔离 Writer Capsule 门：formal Agent writer 必须在仓库外一次性 capsule 中运行，外部 Harness 必须把文件系统限制为 capsule-only，并在 capsule 外生成含绑定 capsule ID 隔离证明的 `novel-forge-runtime/v1` 快照，再经 `record-capsule-runtime` 写入 `.local-guardian/<slug>/runtime/`。writer 只能看到 `capsule.json`、`guardian-contract.json`、`handoff.md`，只能产出 `draft/正文.md`；不得自写 runtime、回执或控制面。额外脚本、软链接/路径逃逸、保护输入变化、缺失或不匹配的隔离证明、session/预算不一致均标记为 compromised，当前 session 失效且不得原地重试。新 capsule 会淘汰旧 prepared capsule；patch 使用正文哈希 CAS。第三个潜在正文版本必须先获得绑定章节、session 与前两份正文哈希的签名授权，导入时再次复核。formal Agent generation 进入 ready 前必须有匹配当前正文、`run_id` 与目标路径的签名 Guardian 回执；公开 evidence 副本不能单独通过。协议不绑定 provider/model/Agent 产品；ACP 与完整 transcript 不是依赖，Guardian 清单、哈希和预算校验在本地完成，不占模型上下文。
...
- `planning/guardian-sessions/*.json` 只能经 Guardian adapter 操作更新；`evidence/guardian-receipts/*.json` 是不可变导入证据，不得覆盖。正式 capsule 必须位于仓库外，且外部 Harness 必须实际限制 writer 文件系统；无法限制时只能进入 `degraded_exploration`。
...
- `docs/`：29 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相、隔离 Writer Capsule 等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
