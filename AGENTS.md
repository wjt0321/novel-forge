...
| `planning_spec.py` | books/ 工作流共享常量唯一来源：场景包必填小节、14 态章节状态链、稿件模式、审稿角色、证据 kind、人类叙事策略 ID、genre 预设 |
...
| `project_templates.py` | `books/<slug>/` 前台项目模板生成（含 voice-bible、八个 agent、evaluation/evidence、reviews 模板、薄壳 tools；genre 预设生效）；不依赖数据库 |
| `cli.py` | 人类命令行入口 |
...
- `books/<slug>/`：**新前台工作流**（`init-novel-project` 生成）。**整个 `books/` 目录已 gitignore，仅存本地不上传**。正文唯一入口是 `chapters/eXX/ch-XX/正文.md`；`memory/canon/**/*.md` 是本书连续性权威源，`memory/candidates/` 是待审增量，`.novel-forge/index.sqlite3` 是按书生成且可删除重建的检索缓存；`evaluation/` 保存评测宪法与实验模板，`evidence/` 保存不可变的生成/盲评/分支/偏好/跨章审计/规则决定；另有 `voice-bible.md`、`planning/`、`reviews/`、`patches/`、薄壳工具、八个 Agent 角色与本书宪法。各书自包含，资产不得跨项目共享。
...
books/ v3.7 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- 审稿角色：`causal-editor`、`line-editor`、`texture-editor`、`consistency-guard`、`blind-reader`、`chapter-editor`；结论绑定当前正文 SHA-256、规划 SHA-256、generation 和模式。第 2 章起 consistency-guard 与 chapter-editor 还绑定上一章正文 SHA-256，任一来源变化后旧审稿自动 stale。
- 进入 `ready` 强制要求正式模式、当前 generation evidence、formal gates 无 blocking、前五个角色 verdict=pass、chapter-editor verdict=ready_for_editor_decision；第 5/10/15…章还要求 `open_must=0` 的 checkpoint arc audit。`ready` 不等于用户批准。
...
- v3.4 有限认知门：决策问题至少两项真实成立；认知账本区分观察/假设/替代解释/可推翻证据；因果归属账本至少一条；专业判断必须登记执行条件与风险或明确豁免；声明有关键对白时对白账本为 blocking。
- v3.2 长篇记忆门：起草前 `memory-status` 必须为 `clean` 并生成本章 context packet；写后新增连续性信息只能先记录为 candidate，显式晋升后才进入 Canon。事实有效期重叠会阻断重建或晋升。
- v3.6 Harness 完整性门：同章同正文 SHA-256 只算一个 generation；重复 ID/阶段不能制造回炉轮次。第四个不同版本在写入前要求 author/human_delegate 的 `human_regeneration_authorized=true` 与决定引用。generation 可记录 `run_id`、`agent_harness`、`reasoning_effort`、`sandbox_profile`、`tool_capabilities` 与 `tool_failures`；降级探索必须记录受限能力和至少一条工具失败，Agent authority 不得自称 `user_attested`。
...
- v3.7 源码卫生与成本短路门：`正文.md` 禁止 Markdown 粗体/强调标记；进入 `surface_checked` 会重跑 blocking lint，失败时不得启动六角色审核或准备下一章。writer 只加载去除模板格式的最小上下文；原始正文默认 standard/medium 推理，Max 主要用于写前反证与 findings 合并，除非用户明确要求推理强度基准实验。
...
- `evidence/**/*.md` 是不可变过程证据；通过 `record-evidence` 创建，不得覆盖。证据、模型评分、gate pass 和 `ready` 都不能声明作者批准或发布许可。
...
- `docs/`：22 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性与源码卫生等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
