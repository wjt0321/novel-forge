...
| `review_capsule.py` | v5.2 仓库外审稿输入封存：按角色最小文件集、正文与 manifest 哈希绑定、回传前完整性复核；Lead 只传路径，不搬运正文 |
...
| `native_relay.py` | v5.2 原生会话拉取/回传协议：Python 持久化下一动作与完成模板；宿主只创建或复用独立 Session、等待官方终态并回传 `role_result`。完成信封可原地补交，真实失败按角色独立重试；默认 `formal_native` 用外置 Capsule、全仓快照、Guardian 和不可变证据闭环，无需命令 Harness。 |
...
- `docs/`：41 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合、每书本地版本历史、读者追读与运行真相、隔离 Writer Capsule、编译 Writer Prompt、自动三角色工作流、文学生产闭环、异步会话完成协议、文学微规则、会话产物封存、Harness 信任、原生角色终态、带类型句柄与角色结果通道、Python 确定性控制、原生 Relay、完成补交与封存审稿等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
