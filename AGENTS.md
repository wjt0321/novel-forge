...
books/ v3.3 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- 章节模式持久化为 `formal` / `exploration`；探索稿可试声、试场景和试分支，但不能进入 `ready`。
- 审稿角色：`causal-editor`、`line-editor`、`texture-editor`、`consistency-guard`、`blind-reader`、`chapter-editor`；结论绑定当前正文 SHA-256、规划 SHA-256、generation 和模式，材料变化后旧审稿自动 stale。
...
- formal narrative_gate 书级材料门：`worldbuilding.md`/`research-boundaries.md` 必须填写或显式标注"无需"；voice-bible 缺失 ch01 advisory、ch02 起 blocking，exemplar_notes 自 ch02 起必填；场景包必须填写“决策问题”和“场景余波”。
- v3.2 长篇记忆门：起草前 `memory-status` 必须为 `clean` 并生成本章 context packet；写后新增连续性信息只能先记录为 candidate，显式晋升后才进入 Canon。事实有效期重叠会阻断重建或晋升。
- v3.3 人类叙事证据门：分支必须引用盲评并选择单一胜者；作者偏好必须引用同一分支/盲评且不得修改 Canon；同模型换角色名不算独立审稿。
...
- `docs/`：18 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测工作流等）+ `docs/examples/`（人味解剖、AI 味反模式与基准分析）+ `superpowers/plans/`。
