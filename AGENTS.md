...
- `books/<slug>/`：**新前台工作流**（`init-novel-project` 生成）。**整个 `books/` 目录已 gitignore，仅存本地不上传**。正文唯一入口是 `chapters/eXX/ch-XX/正文.md`；`memory/canon/**/*.md` 是本书连续性权威源，`memory/candidates/` 是待审增量，`.novel-forge/index.sqlite3` 是按书生成且可删除重建的检索缓存；另有 `voice-bible.md`、`planning/`、`reviews/`、`patches/`、薄壳工具、八个 Agent 角色与本书宪法。各书自包含，资产不得跨项目共享。
