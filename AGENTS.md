...
| `session_audit.py` | v4.2 厂商无关 Harness Contract：定义 `novel-forge-runtime/v1` 累计快照、分层推理策略、硬停预算与每书本地 Git 策略；具体产品日志解析仅为兼容导入 |
...
books/ v4.2 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- `docs/`：27 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、精简文学闭环、外置 Harness 护栏、章节独立会话编排、文学防过拟合与每书本地版本历史等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
