...
| `planning_spec.py` | books/ 工作流共享常量唯一来源：场景包必填小节、8 态章节状态链、默认/按需审稿角色、运行预算、证据 kind、人类叙事策略 ID、genre 预设 |
...
- `books/<slug>/`：**新前台工作流**（`init-novel-project` 生成）。**整个 `books/` 目录已 gitignore，仅存本地不上传**。正文唯一入口是 `chapters/eXX/ch-XX/正文.md`；`memory/canon/**/*.md` 是本书连续性权威源，`memory/candidates/` 是待审增量，`.novel-forge/index.sqlite3` 是按书生成且可删除重建的检索缓存；`evaluation/` 保存评测宪法与实验模板，`evidence/` 保存不可变的生成/盲评/分支/偏好/跨章审计/规则决定；另有 `voice-bible.md`、一页式场景包、`reviews/`、`patches/`、薄壳工具、四个默认 Agent 角色与本书宪法。旧专业审稿角色仍可按需使用，但不再默认生成。各书自包含，资产不得跨项目共享。
...
books/ v3.8 章节状态链（`planning/chapter-state/chXX.md`，由 `advance-state` 校验迁移）：
...
- blind-reader 必须只读当前正文并填写 `human_likeness=convincing|uncertain|synthetic`；只有 convincing 可与 pass 同时成立。进入 `ready` 强制要求正式模式、当前 generation evidence、formal gates 无 blocking、blind-reader=pass、chapter-editor=ready_for_editor_decision；第 5/10/15…章还要求 `open_must=0` 的 checkpoint arc audit。`ready` 不等于用户批准。
...
- v3.8 Harness 完整性与预算门：同章同正文 SHA-256 只算一个 generation；初稿后最多一次集中 patch，第三个不同版本在写入前要求 author/human_delegate 的 `human_regeneration_authorized=true` 与决定引用。generation 可记录 `cached_input_tokens`、`request_count`、`draft_write_count`、`draft_edit_count`、`review_call_count`、`run_id`、`agent_harness`、`reasoning_effort`、`sandbox_profile`、`tool_capabilities` 与 `tool_failures`；运行指标按 generation 增量记录并按章节累计，预算状态区分 `unassessed` / `partial` / `within_budget` / `exceeded`。降级探索必须记录受限能力和至少一条工具失败，Agent authority 不得自称 `user_attested`。
...
- v3.8 文学性与成本短路门：`正文.md` 禁止 Markdown 粗体/强调、`ch05`、`正文.md`、generation、SHA-256、surface_checked、ready 等生产元数据；进入 `surface_checked` 会重跑 blocking lint。`project-status`/`run-gates` 报告跨章精确复读与句长塌缩。writer 每章使用独立会话，只加载一页最小上下文；原始正文默认 standard/medium，Max 仅用于用户明确的基准实验或困难反证。formal ready 使用空值或 `-` 的状态证据会成为 workflow integrity blocker。
...
- `docs/`：23 份里程碑文档（快速开始、数据模型与状态机、质量门控、长篇记忆、人类叙事评测、有限认知、审稿收敛、Harness 完整性、章际连续性、源码卫生与精简文学闭环等）+ `docs/examples/`（人味解剖、AI 味反模式、Agent demo 证据与基准分析）+ `superpowers/plans/`。
