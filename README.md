...
2. Writer 只读取仓库外 Capsule，只写 `draft/正文.md`；Python 先检查 blocking 表面规则，
   有问题就复用同一 Capsule 发回 Writer 修订，通过后再自动导入并建立
...
4. 在独立会话运行 prose-only blind-reader，记录 `human_likeness`、追读意愿与
   ...
   人物可替换性和解释性修补接缝；
5. 有 MUST 时合并为一次证据绑定的集中 Patch，直接发回 Writer 的 patch 动作并优先
   复用当前宿主 Writer 会话，然后全文双审；第二版仍有 MUST 时等待用户选择，只有用户明确重新生成才授权第三版；
