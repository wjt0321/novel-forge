...
- 生命周期句柄必须保留宿主返回的 `kind/value`，不能把 teammate、agent、task 或
  background job 的标识互相猜测。completed 还必须取得
  `novel-forge-role-result/v1`；idle/available 只是可用状态，不是报告。
...
Blind Reader 或 Chapter Editor 已到终态但正式结果未送达、角色绑定错误或报告结构
无效时，也使用新的同角色 session 自动重试两次。Blind 已正式落盘而 Editor 失败时
只重试 Editor；Lead 不读取子会话残留文本代填报告。
