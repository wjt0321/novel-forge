...
`complete-role` 必须携带角色实际使用的宿主会话：`--session-id` 报告真实原生会话
（合成 control id 只用于分发，不能充当完成会话）。Writer 完成时 Python 把章节序列
与 capsule control 重绑到该真实会话，Generation run_id、session-completions 与
Guardian 回执一律只记录真实会话。并行双审下多个未完成角色时，用
`--role blind-reader|chapter-editor` 指明完成对象；Python 对并行完成做
reload+合并写，后完成者不会覆盖先完成者的审稿字段（docs/49 P1-1/P1-2/P1-3）。
...
`uncertain_note` carries the concrete explanation that is mandatory whenever
`human_likeness` is `uncertain` (see the 2026-08-07 rhetorical-layer section).
...
(`literary_revision_required`, `local_patch_hard_gate_failed`,
`hard_gate_failed`, `surface_revision_required`) offer only
`authorize-revision <slug> --reference <依据>` (one concentrated revision plus
a complete re-review, then the same author decision again) and stop, while a
`git_checkpoint_failed` decision offers an authorize-revision retry of only
the ready/checkpoint tail; budget,
...
## 2026-08-02 iteration 47 completion: parallel review and prose retention

The double review now runs in parallel in Lean mode. After the Writer completes, Python issues both the Blind Reader and the Chapter Editor cards up front (`awaiting_double_review`); the host may delegate them concurrently and complete them in either order. The Chapter Editor reviews independently when no Blind Reader conclusion exists yet (`blind_review` is an optional capsule input); the control plane merges both outcomes only after both roles completed. Strict audit keeps the serial blind-then-editor flow. Failure recovery re-queues the exact failed role.

The staged draft (`draft/正文.md`) is never cleared before promotion: capsule re-preparation removes only regenerable auxiliary files, the guardian accepts a capsule that retains the existing prose, and technical retries or regenerations preserve it. The frozen copy is `控制面冻结稿.md`. Control-plane self-managed files (state, relay records, observations) are excluded from snapshot comparison, while per-role cards are protected by recorded digests and action-id consistency checks.

## 2026-08-07 P0-1/P0-2: gate failures reach the author; validation before promotion

Hard-gate failures (formal text gates such as the 5000-CJK minimum, exhausted surface-revision rounds, chapter checkpoint failures) no longer deadlock a chapter. The staged body is checked against the formal text gates at Writer completion, so a too-short chapter loops through the bounded surface-patch rounds before promotion and ends in an authorizable `surface_revision_required` decision. Any `BookProjectError` raised during promotion or finalization routes to an authorizable `hard_gate_failed` decision instead of crashing `complete-role`; `surface_revision_required`, `hard_gate_failed`, and `git_checkpoint_failed` are all accepted by `authorize-revision`. For a checkpoint failure the promoted draft and review evidence are kept and only the ready advance plus the Git checkpoint are retried. If a gate fails after the capsule was already imported, `authorize-revision` refuses the unsafe staged-patch replay with a clear message instead of looping silently. No gate itself was relaxed.

Review certification now runs before promotion: both review outcomes are validated against the staged body (`review_outcome_preflight_errors`, mirroring the model-dependent `record_review` checks — exact evidence-quote substrings, substantive-field placeholders, and the blind-pass `human_likeness=convincing` / `reader_desire=continue` requirements) before the formal chapter, Generation, Guardian receipt, or any Git checkpoint is created. A failed review therefore never leaves formal side effects behind; it routes to the same author decision.

## 2026-08-07 token compression: lean review and writer injections

Lean Chapter Editor inputs are assembled from compressed views instead of raw control-plane dumps. Canon now arrives as the `build_context_packet` digest (hard facts, active narrative, and due promises in the compact per-record line format, still capped at 12,000 characters) via the shared `book_memory.canon_context_digest` helper, replacing the raw `memory/canon/**/*.md` concatenation that was dominated by record metadata and could truncate away the newest facts. The scene package input keeps only the counterpart sections (`EDITOR_SCENE_SECTIONS`, derived from `WRITER_VISIBLE_SCENE_SECTIONS` minus the story contract); the falsification, cognition-alternative, causal-responsibility, and expertise-audit sections stay in the file but out of the capsule, and the user story contract appears exactly once, in its dedicated `story_contract` input. The previous-chapter ending window narrows from the last 20% to the last 10% of the promoted prose, which still covers the closing beat and hook needed for the continuity check.

Writer package assembly drops two dead slots. The 0b handoff ledger lines (previous-chapter path and SHA-256) no longer enter the P0 story obligations; only the 48-character closing quote remains, while the Scene Package file itself keeps the ledger for gates and editors. The P1 "直接 Canon" slot no longer reads the never-created `memory/characters.md`/`memory/canon.md`; it now carries the compressed hard/active canon digest ahead of `memory/worldbuilding.md`, still bounded at 220 CJK inside the unchanged 850-CJK P1 budget.

Minimal (lean) Writer capsules no longer embed `handoff.md`. The bounded handoff stays at its canonical `memory/context-cache/` path; `capsule.json` records `handoff_embedded=false` plus the book-relative `handoff_path`, the file leaves the protected-input set, and ingest re-validates the source handoff hash (a mismatch compromises the capsule as `handoff_source_changed`). Full and strict-audit capsules still embed the handoff unchanged, and strict-audit review inputs keep the raw canon concatenation, the full scene package, and the 20% ending window.

## 2026-08-07 rhetorical-layer advisories and the uncertain contract (docs/49 §4)

The prose lint layer now reaches below the sentence level. `mechanical-triplet` additionally flags clause-level anaphora (three or more consecutive comma clauses sharing the same ≥2-character CJK opening, e.g. "那些A，那些B，那些C") and ≥2-character reduplication chains ("他沉默了，沉默得让人心慌"); two-part parallelism stays allowed. A new advisory rule `not-only-flip` covers the "不仅/不仅仅/不只是 X，而/更/而是 Y" elevation tic with the same question and dialogue exemptions as `not-is-flip`, which stays blocking. `explanation-tic` gains the viewpoint-leak and stock-imagery patterns "她不知道的是 / 没人知道 / 命运的(车轮|齿轮|轨迹) / 时间仿佛(凝固|静止) / 空气中弥漫着 / 说不清道不明", and `simile-density` no longer skips fragments under 500 CJK. All of these remain advisory; one observation round will review their false-positive rate before any severity change. The `literary_texture` repeated-n-gram window drops to 2–3 characters with higher repeat thresholds (8 for bigrams, 6 for trigrams, versus 4 for longer grams) plus function-word exclusion, so refrains like "沉默……沉默" surface as repeated short phrases.

Lint advisories now reach the review they were meant to inform. `summarize_advisories` aggregates `explanation-tic`, `rhythm-monotony`, `mechanical-triplet`, and `simile-density` findings per rule into a ≤240-character sample that joins the ≤160-character texture hint as one `machine_diagnostics` input (≤400 characters total) in the Lean Chapter Editor capsule. The semantics are unchanged: a low-cost sample, never a standalone verdict, to be checked for chapter-wide spread. The Blind Reader capsule stays prose-only, and the strict-audit path keeps its gate-run diagnostics.

The `uncertain` contract is unified with the code (P0-3), superseding the "uncertain 默认不触发修订" sentence in the 2026-07-31 section: `uncertain` is not a pass — a blind pass still requires `human_likeness=convincing` and `reader_desire=continue` — and every `uncertain` result must carry a concrete `uncertain_note` naming the passage that reads generic, tidy, or over-explained. An empty note invalidates the result. The check lives in `record_review` (`_review_validation_errors`), with `review_outcome_preflight_errors` kept in sync so a failed review still produces no formal side effects.

Review evidence checks extend from the headline quote to every open MUST: `_assert_review_evidence_quote` now runs the same bounded-tolerance `_quote_matches` against each MUST finding's `evidence`, so a fabricated citation invalidates the result and routes to the same repair path as an invalid `evidence_quote`.

Both patch instruction channels carry one new hard constraint — new causality must land in action, pause, or object consequence, and new explanatory paragraphs are forbidden — covering the chapter-level patch (`writer_prompt.py`) and the local-replacement patch (`local-patch/input.json` instruction).
