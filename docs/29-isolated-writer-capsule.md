...
v4.5 起，`instructions.md` 由 Guardian 按 `formal-writer/v1` 编译，包含当前章范围、
完整章节目标、5000 CJK 下限、唯一输出与停止条件。它与 handoff 一样是受保护输入；
writer 不接收完整 Skill、验证器实现或多章旧上下文。
...
- `capsule.json`、合同、instructions 或 handoff 被修改；
...
预算和回执校验全部在本地 Python 中执行，不占模型上下文。模型只读取一次编译短
提示词与有界 handoff；runtime 只保存累计计数和来源元数据，不保存 prompt、正文、
思考链或工具返回。提示词只在 generation/回执中保存模板 ID 与 SHA-256。
