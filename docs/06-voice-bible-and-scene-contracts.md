...
## Scene Contract v4（章节级）

路径：`library/<slug>/planning/chapters/ch<NNNN>-contract/revisions/`

创建章节时自动生成 revision 1 模板（当前为 v4）。后续通过 `write-scene-contract` 写入新 revision。旧 v2/v3 合同不会被强制迁移，仍可读取，但会触发建议升级到 v4 的 warning。
...
| 字段 | 含义 | 填写原则 |
|------|------|----------|
| spatial_layout_and_routes | 空间布局与路线 | 人物、出口、关键物体的相对位置与可通行路线；禁止只写面积/尺寸参数 |
| body_state_and_contacts | 身体状态与接触 | 开场身体姿态、负重/疼痛/衣物、正在接触或避让的表面/物体 |
| object_affordances | 物体可供性 | 最多 5 个关键物体，每项写"物体 → 可做什么/不可做什么 → 何时改变行动" |
| environmental_constraints | 环境约束 | 环境怎样具体阻挡、拖慢、暴露或伤害动作；至少一条因果链 |
| embodied_action_chain | 具身动作链 | 至少 3 步"感知/接触 → 动作 → 环境反馈/代价"，必须覆盖 `irreversible_choice` 或其前置动作 |
...
- ❌ "房间 20 平方米，长 5 米、宽 4 米"
- ✅ "spatial_layout_and_routes：主角背对锁死的门，左手三步外是碎玻璃窗台，窗台与门之间散落着椅子的残骸"

- ❌ "环境很危险"
- ✅ "environmental_constraints：地板潮湿 → 快步会打滑 → 她必须扶住窗台，从而把身体暴露在窗外光线中"
